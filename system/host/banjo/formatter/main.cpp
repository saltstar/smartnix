...
bool Format(const banjo::SourceFile& source_file, banjo::IdentifierTable* identifier_table,
            banjo::ErrorReporter* error_reporter, std::string& output) {
    banjo::Lexer lexer(source_file, identifier_table);
    banjo::Parser parser(&lexer, error_reporter);
    std::unique_ptr<banjo::raw::File> ast = parser.Parse();
    if (!parser.Ok()) {
        return false;
    }
    banjo::raw::FormattingTreeVisitor visitor;
    visitor.OnFile(ast);
...
int main(int argc, char* argv[]) {
    ...
    banjo::SourceManager source_manager;
