...
TypeShape BanjoStructTypeShape(std::vector<FieldShape*>* fields) {
    return CStructTypeShape(fields);
}

TypeShape PointerTypeShape(TypeShape element, uint32_t max_element_count = 1u) {
    // Because BANJO supports recursive data structures, we might not have
...
bool HasAttributeHelper(const raw::AttributeList* attributes, banjo::StringView name) {
    if (!attributes)
        return false;
    return attributes->HasAttribute(name);
}

banjo::StringView GetAttributeHelper(const raw::AttributeList* attributes,
                                    banjo::StringView name) {
    if (!attributes)
        return banjo::StringView();

    for (const auto& attribute : attributes->attributes_->attributes_) {
        if (StringView(attribute->name) == name) {
            if (attribute->value != "") {
                const auto& value = attribute->value;
                return banjo::StringView(value.data(), value.size());
            }
...
bool Decl::HasAttribute(banjo::StringView name) const {
    return HasAttributeHelper(attributes.get(), name);
}

banjo::StringView Decl::GetAttribute(banjo::StringView name) const {
...
bool Enum::Member::HasAttribute(banjo::StringView name) const {
    return HasAttributeHelper(attributes.get(), name);
}

banjo::StringView Enum::Member::GetAttribute(banjo::StringView name) const {
    return GetAttributeHelper(attributes.get(), name);
}

bool Interface::Method::HasAttribute(banjo::StringView name) const {
    return HasAttributeHelper(attributes.get(), name);
}

banjo::StringView Interface::Method::GetAttribute(banjo::StringView name) const {
    return GetAttributeHelper(attributes.get(), name);
}

bool Struct::Member::HasAttribute(banjo::StringView name) const {
    return HasAttributeHelper(attributes.get(), name);
}

banjo::StringView Struct::Member::GetAttribute(banjo::StringView name) const {
    return GetAttributeHelper(attributes.get(), name);
}

bool Union::Member::HasAttribute(banjo::StringView name) const {
    return HasAttributeHelper(attributes.get(), name);
}

banjo::StringView Union::Member::GetAttribute(banjo::StringView name) const {
    return GetAttributeHelper(attributes.get(), name);
}

bool Libraries::Insert(std::unique_ptr<Library> library) {
    std::vector<banjo::StringView> library_name = library->name();
...
bool Library::ConsumeFile(std::unique_ptr<raw::File> file) {
    ...
    // All banjo files in a library should agree on the library name.
...
bool Library::CompileInterface(Interface* interface_declaration) {
    ...
    for (auto& method : interface_declaration->methods) {
        auto CreateMessage = [&](Interface::Method::Message* message) -> bool {
            ...
            message->typeshape = BanjoStructTypeShape(&message_struct);
            return true;
        };
...
bool Library::CompileStruct(Struct* struct_declaration) {
    ...
    Scope<StringView> scope;
    std::vector<FieldShape*> banjo_struct;
    ...
    for (auto& member : struct_declaration->members) {
        ...
        if (!CompileType(member.type.get(), &member.fieldshape.Typeshape()))
            return false;
        banjo_struct.push_back(&member.fieldshape);
    }
    ...
    struct_declaration->typeshape = CStructTypeShape(&banjo_struct, max_member_handles);
...
bool Library::CompileUnion(Union* union_declaration) {
    ...
    std::vector<FieldShape*> banjo_union = {&tag, &union_declaration->membershape};
    union_declaration->typeshape = CStructTypeShape(&banjo_union, extra_handles);
...
bool Library::HasAttribute(banjo::StringView name) const {
    if (!attributes_)
        return false;
