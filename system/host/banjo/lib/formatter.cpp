...
// The implementation for the FormattingTreeVisitor that pretty-prints BANJO code.
...
void FormattingTreeVisitor::OnFile(std::unique_ptr<banjo::raw::File> const& element) {
    // Eat ws at the beginning of the file.
    banjo::Token real_start = element->start_;
    banjo::StringView start_view = real_start.previous_end().data();
    const char* start_ptr = start_view.data();
    int initial_length = start_view.size();
    size_t offset = strspn(start_ptr, kWsCharacters);
    banjo::StringView processed_file_start(
        start_ptr + offset, initial_length - offset);
    element->start_.set_previous_end(
        banjo::SourceLocation(processed_file_start, real_start.previous_end().source_file()));

    banjo::raw::DeclarationOrderTreeVisitor::OnFile(element);
}
